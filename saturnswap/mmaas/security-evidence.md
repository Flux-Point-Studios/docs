...
Our security evidence for MMaaS is internal. Red-team runs by Claude, Codex and Kimi models attack
the protocol, and we investigate their findings and re-test for regressions. Validator tests are
recorded against a named source revision, below. This is internal evidence: it is not external
certification, and it is not a guarantee that no vulnerabilities remain.
