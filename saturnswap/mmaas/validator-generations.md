...
These are generation-specific guarantees, not a claim that a generation has no other
defect. The band is an immutable bid ceiling and ask floor, not an oracle-relative price
guarantee. Public taker fills run the DEX's trading rules and are distinct from bot owner
actions.
